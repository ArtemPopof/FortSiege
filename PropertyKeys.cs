public class PropertyKeys {
    public const string COIN_COUNT = "Coin_count";
    public const string WEAPON_POSSESION_LIST = "WeaponPossesion";
    public const string AVAILABLE_LEVELS = "AvailableLevel";
    public const string AVAILABLE_EXTENTIONS = "AvailableExtention";
    public const string CURRENT_WEAPON = "CurrentWeapon";
    public const string CURRENT_LEVEL = "currentLevel";
    public const string WEAPON_POWER_MODIFICATION = "PowerModification";
    public const string WEAPON_WEIGHT_MODIFICATION = "WeightModification";
    public const string WEAPON_SHOT_COUNT_MODIFICATION = "ShotCountModification";

    public const string SCENE_ARG_1 = "SceneArg1";
}